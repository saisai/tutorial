class Sudoku {
    ...
    // Sudoku class to build hold the board and related functions
    constructor(board){
        this.board = board
    }
    ...
    findEmptyCell(){
        // find an empty cell in the board ( retuns [-1, -1] if all cells are filed )
        for(let i=0; i < 9; i++){
            for(let j=0; j < 0; j++){
    ...
    check([y, x], value){
        ...
        for(let i = (secRow * 3); i < ((secRow*3) + 3); i++){
            for(let j=(secCol*3); j < ((secCol * 3) + 3); j++){
                if(y !== i && x !== j
                && this.board[i][j] === value) return false
            }
        }
