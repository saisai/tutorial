...
class Solution:
    ...
    def find_sub_string(self, s, words):
        ...
        for stripe in range(word_len):  # each stripe starts at a different position in s, modulo word_len
            ...
            while i + to_match*word_len <= len(s):  # the next index in s that we want to match a word
...
if __name__ == '__main__':
    ...
    result = s.find_sub_string("wordgoodgoodgoodbestword", ["word","good","best","word"])
    print(result)

    result = s.find_sub_string("barfoothefoobarman", ["foo","bar"])
    print(result)
